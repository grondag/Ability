...
public class SquareColumnCapped extends FormedNonCubicPillarBlock {
	public SquareColumnCapped(Properties settings, PrimitiveState defaultModelState, PrimitiveStateMutator stateFunc) {
		super(settings, defaultModelState, stateFunc);
	}

	public static SquareColumnCapped create(FormedBlockMaterial material) {
		final var defaultState = CappedSquareInsetColumn.INSTANCE.newState()
		...
		return new SquareColumnCapped(material.settings(), defaultState.releaseToImmutable(), AXIS_JOIN_COLUMN_MUTATOR);
	}
}
