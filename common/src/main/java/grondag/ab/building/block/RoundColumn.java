...
public class RoundColumn extends FormedNonCubicPillarBlock {
	public RoundColumn(Properties settings, PrimitiveState defaultModelState, PrimitiveStateMutator stateFunc) {
		super(settings, defaultModelState, stateFunc);
	}

	public static RoundColumn create(FormedBlockMaterial material) {
		final var defaultState = CylinderWithAxis.INSTANCE.newState()
		...
		return new RoundColumn(material.settings(), defaultState.releaseToImmutable(), stateFunc);
	}
}
