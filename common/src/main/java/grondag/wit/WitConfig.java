...
public class WitConfig {
	...
	public static void initialize() {
		configFile = new File(FabricLoader.getInstance().getConfigDir().toFile(), "wit.json5");
	...
	private static void loadConfig() {
		...
		try {
			...
			e.printStackTrace();
			Wit.LOG.error("Unable to load config. Using default values.");
		}
	...
	public static void saveConfig() {
		...
		try {
			...
			e.printStackTrace();
			Wit.LOG.error("Unable to save config.");
			return;
		}
	}
}
