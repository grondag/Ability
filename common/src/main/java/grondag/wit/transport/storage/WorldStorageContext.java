...
package grondag.wit.transport.storage;
...
public abstract class WorldStorageContext implements TransportStorageContext {
	...
	protected static final int FLUID_COOLDOWN_TICKS = 64 / WitConfig.utb1ItemsPerTick;
	...
	public long capacityFor(Article article, long divisor) {
		...
		if (article.isFluid()) {
			...
			return canDropItems ? WitConfig.utb1ItemsPerTick : 0;
		} else {
			return 0;
		}
	}
	...
	public long accept(Article article, long numerator, long divisor) {
		...
		if (article.isFluid()) {
			...
			if (canDropItems) {
				final long howMany = Math.min(numerator, WitConfig.utb1ItemsPerTick);
